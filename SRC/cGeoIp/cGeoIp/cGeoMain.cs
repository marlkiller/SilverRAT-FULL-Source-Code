...
public class cGeoMain
{
	...
	public cGeoMain()
	{
		...
		getPICs = new cFake();
		cImageList = getPICs.cImgList;
		MS = new MemoryStream(Resources.GeoIP);
	}
	...
	public string GetIpInf(string IPA)
	{
		...
		result = checked(IPA + ":" + CountryName[(int)SeekCountry(0L, ConvertIPAddressToNumber(iPA), 31)] + ":" + CountryCode[(int)SeekCountry(0L, ConvertIPAddressToNumber(iPA), 31)]);
		goto IL_0075;
		IL_0075:
		return result;
	}
	...
	private long SeekCountry(long StartOffset, long IPNumber, int SearchDepth)
	{
		...
		checked
		{
			try
			{
				MS.Seek(6 * StartOffset, SeekOrigin.Begin);
				MS.Read(array, 0, 6);
			}
			...
			while (num <= 1);
			if ((IPNumber & vbShiftLeft(1L, SearchDepth)) > 0)
			{
